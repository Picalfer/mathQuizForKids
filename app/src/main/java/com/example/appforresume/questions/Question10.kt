...
import com.example.appforresume.databinding.FragmentQuestion10Binding

class Question10 : Fragment() {
    private val dataModel: DataModel by activityViewModels()
    private lateinit var b: FragmentQuestion10Binding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View {
        b = FragmentQuestion10Binding.inflate(inflater)
        return b.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) = with(b) {
        ...
        checkOut.setOnClickListener {
            if (!isFieldEmpty()) {
                result = if (answer.text.toString() == "3") {
        ...
        toNext.setOnClickListener {
            activity?.supportFragmentManager?.beginTransaction()?.remove(this@Question10)?.commit()
