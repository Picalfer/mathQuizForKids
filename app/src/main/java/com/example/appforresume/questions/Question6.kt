...
class Question6 : Fragment() {
    ...
    private lateinit var b: FragmentQuestion6Binding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View {
        b = FragmentQuestion6Binding.inflate(inflater)
        return b.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) = with(b) {
        ...
        checkOut.setOnClickListener {
            if (!isFieldEmpty()) {
                result = if (answer.text.toString() == "50") {
        ...
        toNext.setOnClickListener {
            activity?.supportFragmentManager?.beginTransaction()?.remove(this@Question6)?.commit()
