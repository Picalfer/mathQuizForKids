...
class Question9 : Fragment() {
    ...
    private lateinit var b: FragmentQuestion9Binding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) = with(b) {
        ...
        checkOut.setOnClickListener {
            if (!isFieldEmpty()) {
                result = if (answer.text.toString() == "0") {
        ...
        toNext.setOnClickListener {
            activity?.supportFragmentManager?.beginTransaction()?.remove(this@Question9)?.commit()
